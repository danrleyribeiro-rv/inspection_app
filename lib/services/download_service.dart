...
class DownloadService {
  ...
  Future<bool> downloadInspection(String inspectionId, {Function(double)? onProgress}) async {
    try {
      debugPrint('DownloadService: Starting download of inspection $inspectionId');
      
      if (onProgress != null) onProgress(0.1);
      
      // Step 1: Check if inspection already exists locally
      final existingInspection = await _serviceFactory.dataService.getInspection(inspectionId);
      ...
      try {
        ...
        final inspectionData = await _serviceFactory.dataService.getInspection(inspectionId);
        if (inspectionData == null) {
          debugPrint('DownloadService: Failed to download inspection from Firestore');
          return false;
        }
      } catch (e) {
      ...
      debugPrint('DownloadService: Inspection data saved locally via sync service');
      ...
      debugPrint('DownloadService: Successfully downloaded inspection $inspectionId');
      return true;
    } catch (e) {
      debugPrint('DownloadService: Error downloading inspection: $e');
      return false;
  ...
  Future<void> downloadInspectionForOfflineEditing(String inspectionId) async {
    debugPrint('DownloadService: downloadInspectionForOfflineEditing called - redirecting to new system');
    final result = await downloadInspection(inspectionId);
    if (result) {
      debugPrint('DownloadService: Successfully downloaded inspection for offline editing');
    } else {
      debugPrint('DownloadService: Failed to download inspection for offline editing');
    }
  }

  /// Check if inspection is downloaded
  Future<bool> isInspectionDownloaded(String inspectionId) async {
    debugPrint('DownloadService: isInspectionDownloaded called - redirecting to new system');
  ...
  Future<List<Map<String, dynamic>>> getAvailableInspections() async {
    debugPrint('DownloadService: getAvailableInspections called - returning empty list (deprecated)');
    return [];
  }

  /// Returns true for compatibility - deprecated method
  Future<bool> isInspectionAvailable(String inspectionId) async {
    debugPrint('DownloadService: isInspectionAvailable called - using new system');
    final inspection = await _serviceFactory.dataService.getInspection(inspectionId);
    return inspection != null;
  }
}
