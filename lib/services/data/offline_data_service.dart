...
class OfflineDataService {
  ...
  Future<void> updateInspectionData(
      String inspectionId, Map<String, dynamic> updates) async {
  ...
  Future<void> addTopic(
      String inspectionId, Map<String, dynamic> topicData) async {
    final inspection = await _storage.getInspection(inspectionId);
    if (inspection == null) {
      throw Exception('Inspection not found: $inspectionId');
    }
  ...
  Future<void> updateTopic(String inspectionId, int topicIndex,
    ...
    await updateInspectionData(inspectionId, {'topics': topics});
    debugPrint(
        'OfflineDataService: Updated topic $topicIndex in inspection $inspectionId');
  }
  ...
  Future<void> addItem(String inspectionId, int topicIndex,
    ...
    final items =
        List<Map<String, dynamic>>.from(topics[topicIndex]['items'] ?? []);
    items.add(itemData);

    topics[topicIndex]['items'] = items;
    await updateInspectionData(inspectionId, {'topics': topics});
    debugPrint(
        'OfflineDataService: Added item to topic $topicIndex in inspection $inspectionId');
  }
  ...
  Future<void> updateItem(String inspectionId, int topicIndex, int itemIndex,
    ...
    await updateInspectionData(inspectionId, {'topics': topics});
    debugPrint(
        'OfflineDataService: Updated item $itemIndex in topic $topicIndex in inspection $inspectionId');
  }
  ...
  Future<void> updateDetail(String inspectionId, int topicIndex, int itemIndex,
      int detailIndex, Map<String, dynamic> detailData) async {
    final inspection = await _storage.getInspection(inspectionId);
    if (inspection == null) {
      throw Exception('Inspection not found: $inspectionId');
    }
    ...
    final details =
        List<Map<String, dynamic>>.from(items[itemIndex]['details'] ?? []);
    ...
    await updateInspectionData(inspectionId, {'topics': topics});
    debugPrint(
        'OfflineDataService: Updated detail $detailIndex in item $itemIndex in topic $topicIndex in inspection $inspectionId');
  }
  ...
  Future<void> updateDetailMedia(String inspectionId, String topicId,
    ...
    await _storage.saveInspection(updatedInspection);
    debugPrint(
        'OfflineDataService: Updated media for detail $detailId in inspection $inspectionId');
  }
