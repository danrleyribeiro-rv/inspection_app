...
import 'package:inspection_app/services/features/enhanced_offline_media_service.dart';
import 'package:inspection_app/services/sync/firestore_sync_service.dart';
...
class EnhancedOfflineServiceFactory {
  ...
  static EnhancedOfflineServiceFactory get instance => _instance ??= EnhancedOfflineServiceFactory._();
  ...
  void _checkInitialization() {
    if (!_isInitialized) {
      throw StateError('EnhancedOfflineServiceFactory not initialized. Call initialize() first.');
    }
  }
  ...
  Future<void> initialize() async {
    ...
    try {
      debugPrint('EnhancedOfflineServiceFactory: Initializing enhanced offline services...');
      ...
      debugPrint('EnhancedOfflineServiceFactory: Enhanced data service initialized');
      
      // 5. Inicializar Enhanced Media Service
      _mediaService = EnhancedOfflineMediaService.instance;
      await _mediaService!.initialize();
      debugPrint('EnhancedOfflineServiceFactory: Enhanced media service initialized');
      
      // 6. Inicializar Sync Service
      ...
      debugPrint('EnhancedOfflineServiceFactory: SQLite storage service initialized');
      
      _isInitialized = true;
      debugPrint('EnhancedOfflineServiceFactory: All enhanced offline services initialized successfully');
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getGlobalStats() async {
    try {
      ...
      debugPrint('EnhancedOfflineServiceFactory: Error getting global stats: $e');
      return {
        'error': e.toString(),
        'timestamp': DateTime.now().toIso8601String(),
      };
  ...
  Future<Map<String, dynamic>> getInspectionStats(String inspectionId) async {
    try {
      ...
      final dataStats = await _dataService!.getInspectionCompleteStats(inspectionId);
      final mediaStats = await _mediaService!.getMediaStats(inspectionId);
      final totalMediaSize = await _mediaService!.getTotalMediaSize(inspectionId);
      ...
      debugPrint('EnhancedOfflineServiceFactory: Error getting inspection stats: $e');
      return {
        'error': e.toString(),
        'inspection_id': inspectionId,
        'timestamp': DateTime.now().toIso8601String(),
  ...
  Future<Map<String, dynamic>> getConnectivityStatus() async {
    try {
      ...
      debugPrint('EnhancedOfflineServiceFactory: Error getting connectivity status: $e');
      return {
        'error': e.toString(),
        'timestamp': DateTime.now().toIso8601String(),
      };
  ...
  Future<void> syncInspection(String inspectionId) async {
    try {
      ...
      debugPrint('EnhancedOfflineServiceFactory: Starting sync for inspection $inspectionId...');
      
      await _syncService!.syncInspection(inspectionId);
      
      debugPrint('EnhancedOfflineServiceFactory: Sync completed for inspection $inspectionId');
    } catch (e) {
      debugPrint('EnhancedOfflineServiceFactory: Error syncing inspection $inspectionId: $e');
      rethrow;
    }
  }
  ...
  Future<void> optimizeDatabase() async {
    try {
      ...
      debugPrint('EnhancedOfflineServiceFactory: Database optimization completed');
    } catch (e) {
      debugPrint('EnhancedOfflineServiceFactory: Error optimizing database: $e');
      rethrow;
    }
  }
