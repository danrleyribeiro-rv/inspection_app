...
class _TopicWidgetState extends State<TopicWidget> {
  ...
  Future<void> _loadItems() async {
    ...
    try {
      ...
      final items = await _offlineService.getItems(
  ...
  Future<void> _addItem() async {
    ...
    try {
      ...
      final newItem = await _offlineService.addItem(
        widget.topic.inspectionId,
        widget.topic.id!,
        itemName,
        label: itemLabel,
      ...
      if (template['isCustom'] != true && template['observation'] != null) {
        ...
        await _offlineService.updateItem(updatedItem);
      }
  ...
  Future<void> _duplicateItem(Item item) async {
    ...
    try {
      await _offlineService.isItemDuplicate(
        widget.topic.inspectionId,
        widget.topic.id!,
        item.itemName,
      );
  ...
  void _handleItemUpdate(Item updatedItem) {
    ...
    if (index >= 0) {
      ...
      _offlineService.updateItem(updatedItem);
    }
  }
  ...
  Future<void> _handleItemDelete(dynamic itemId) async {
    try {
      ...
      await _offlineService.deleteItem(
        widget.topic.inspectionId,
        widget.topic.id!,
        itemId,
      );
