// lib/presentation/screens/inspection/improved_item_widget.dart
import 'package:flutter/material.dart';
import 'package:inspection_app/models/item.dart';
import 'package:inspection_app/models/detail.dart';
import 'package:inspection_app/presentation/screens/inspection/improved_detail_widget.dart';
import 'package:inspection_app/services/inspection_service.dart';

class ImprovedItemWidget extends StatefulWidget {
  final Item item;
  ...
  @override
  State<ImprovedItemWidget> createState() => _ImprovedItemWidgetState();
}

class _ImprovedItemWidgetState extends State<ImprovedItemWidget> {
  ...
  Future<void> _addDetail() async {
    ...
    List<String> existingDetailNames = _details.map((d) => d.detailName).toList();
    ...
    if (availableTemplates.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('All available details have been added')),
        );
      }
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    int filledDetails = _details.where((d) => 
        (d.detailValue != null && d.detailValue!.isNotEmpty) || 
        (d.observation != null && d.observation!.isNotEmpty)).length;
    
    double progress = totalDetails > 0 ? filledDetails / totalDetails : 0.0;
    
    // Check if item itself has data filled
    bool isItemFilled = (widget.item.observation != null && widget.item.observation!.isNotEmpty);
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: isItemFilled ? Colors.blue.withOpacity(0.1) : Colors.grey.withOpacity(0.1),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (widget.item.itemLabel != null && widget.item.itemLabel!.isNotEmpty)
                    Padding(
                  ...
                  if (_isLoading)
                    ...
                    const Center(
                      child: Padding(
                        ...
                        child: Text('No details added yet. Click "Add Detail" to begin.'),
                      ),
                    )
                  else
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        // Find the template for this detail
                        final detailTemplate = (widget.itemTemplate['details'] as List?)?.firstWhere(
                          (t) => t['name'] == detail.detailName,
                          orElse: () => {'type': 'text'}, // Default to text if template not found
                        );
                        
                        return ImprovedDetailWidget(
                          detail: detail,
